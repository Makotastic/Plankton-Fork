...
class VelocityControllerNode(Node):
    def __init__(self, name, **kwargs):
        ...
        self._declare_and_fill_map(
            "linear_p", 1.0, "p component of pid for linear vel.", self.config
        )
        self._declare_and_fill_map(
            "linear_i", 0.0, "i component of pid for linear vel.", self.config
        )
        self._declare_and_fill_map(
            "linear_d", 0.0, "d component of pid for linear vel.", self.config
        )
        self._declare_and_fill_map(
            "linear_sat", 1.0, "saturation of pid for linear vel.", self.config
        )

        self._declare_and_fill_map(
            "angular_p", 1.0, "p component of pid for angular vel.", self.config
        )
        self._declare_and_fill_map(
        ...
        self.sub_cmd_vel = self.create_subscription(
            geometry_msgs.Twist, "cmd_vel", self.cmd_vel_callback, 10
        )
